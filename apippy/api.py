...
def get_price_minute(securitys=None, type='stock',
                     start_date=None, end_date=None, frequency='daily', fields=None, fq='pre', engine='thread'):
    assert type in PRICE_TYPE, f'type must be in {PRICE_TYPE}'
    assert engine in ENGINE_TYPE, f'engine must be in {ENGINE_TYPE}'
    base_path = ""
    if type == 'stock':
        assert frequency in FREQUENCY_TYPE, f'frequency must be in {FREQUENCY_TYPE}'
        if frequency == 'minute':
            base_path = '/mnt/data/hub/stock/stock_quote/stock_15m'
    elif type == 'index':
        if frequency == 'minute':
            base_path = '/mnt/data/hub/index/minute_quote'
        if frequency == 'daily':
            base_path = '/mnt/data/hub/index/daily_quote'
    elif type == 'future':
        base_path = '/mnt/data/hub/future/future_quote/daily_quote'
